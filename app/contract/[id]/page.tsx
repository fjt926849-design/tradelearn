...
export default async function ContractDetailPage({
  ...
  const sameCategoryConcepts = contractConcepts.filter(
    (c) => c.category === concept.category && c.id !== concept.id
  );

  const nav = getModuleNav("contract");
  const index = nav.findIndex((n) => n.slug === id);
  const { prev, next } = getPrevNext("contract", id);
        ...
        <div className="lg:grid lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-10 lg:items-start">
          ...
          <div className="min-w-0">
            <nav className="flex items-center gap-1.5 text-sm mb-8" style={{ color: "var(--color-text-muted)" }}>
              ...
              <span>/</span>
              <Link href="/contract" className="hover:text-black transition-colors">合同条款</Link>
              <span>/</span>
            ...
            <div className="space-y-8">
              <div>
                <p className="text-xs font-semibold tracking-[0.16em]" style={{ color: "var(--color-accent)" }}>
                  {MODULE_CHAPTERS.contract.no} · {MODULE_CHAPTERS.contract.en} · {String(index + 1).padStart(2, "0")} / {String(nav.length).padStart(2, "0")}
                </p>
                ...
                <div>
                  ...
                  <div className="mt-4 space-y-4">
                    {concept.comparisons.map((comp) => {
                      const related = contractConcepts.find((c) => c.id === comp.conceptId);
                      return (
                        <div key={comp.conceptId} className="border rounded-lg p-4" style={{ borderColor: "var(--color-border)" }}>
                          <div className="flex items-center gap-2 mb-2">
                            {related && (
                              <Link href={`/contract/${comp.conceptId}`} className="inline-flex items-center gap-1.5 text-sm font-medium hover:underline" style={{ color: "var(--color-text)" }}>
              ...
              {relatedIncoterms.length > 0 && (
                <div>
                  ...
                  <p className="mt-3 text-xs" style={{ color: "var(--color-text-muted)" }}>实务中与本条款相关的 Incoterms：</p>
            ...
            <div className="mt-10 pt-8 border-t space-y-5" style={{ borderColor: "var(--color-border)" }}>
              <div className="flex items-center justify-between">
                ...
                <Link href="/contract/flashcards" className="inline-flex items-center px-4 py-2 text-sm font-medium rounded-md border transition-colors hover:bg-gray-50" style={{ color: "var(--color-text)", borderColor: "var(--color-text)" }}>
                  开始闪卡复习 →
                </Link>
              </div>
                ...
                <div>
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {sameCategoryConcepts.map((sc) => (
                      <Link key={sc.id} href={`/contract/${sc.id}`} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm border rounded-md hover:bg-gray-50 transition-colors" style={{ borderColor: "var(--color-border)" }}>
