    ...
    <>
      ...
      <main className="flex-1">
        ...
        <div className="mx-auto max-w-6xl space-y-10 px-5 py-10">
          ...
          </section> : <section className="rounded-xl border p-6" style={{ borderColor: "var(--color-border)", background: "var(--color-bg-soft)" }}><p className="text-xs font-semibold uppercase tracking-[0.18em]" style={{ color: "var(--color-accent)" }}>WELCOME · 新用户引导</p><h2 className="mt-2 text-lg font-semibold">从第一张术语卡片开始</h2><p className="mt-2 max-w-2xl text-sm leading-6" style={{ color: "var(--color-text-secondary)" }}>先浏览贸易术语卡片，熟悉缩写、中文含义和典型业务场景，再按需要进入合同、运输和保险模块。</p><div className="mt-5 flex flex-wrap gap-2"><Link href="/terms-preview" className="rounded-md px-4 py-2 text-sm font-medium text-white" style={{ background: "var(--color-accent)" }}>浏览术语卡片</Link><Link href="/practice" className="rounded-md border px-4 py-2 text-sm" style={{ borderColor: "var(--color-border)" }}>查看业务实战</Link></div></section>}
