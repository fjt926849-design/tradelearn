            ...
            <div className="grid lg:grid-cols-[1.15fr_0.85fr]">
              <div className="p-7 sm:p-10 lg:p-14">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  ...
                  <Link
                    href="/flashcards"
                    className="rounded-lg border px-5 py-3 text-sm font-medium transition-colors hover:bg-gray-50"
              ...
              <div className="border-t p-7 sm:p-10 lg:border-l lg:border-t-0 lg:p-12" style={{ borderColor: "var(--color-border)", background: "rgba(247,249,252,0.68)" }}>
                <p className="text-xs font-semibold tracking-[0.24em]" style={{ color: "var(--color-text-muted)" }}>
                  {isLearning ? "CONTINUE LEARNING" : "START HERE"}
                </p>
                <h2 className="mt-3 text-xl font-semibold">{isLearning ? "继续学习这张术语卡" : "从这张术语卡开始"}</h2>
                <Link
                  href={continueTerm.href}
                  className="mt-6 block rounded-2xl border p-6 transition-transform hover:-translate-y-0.5"
                  style={{ borderColor: "var(--color-border)", background: "#fff", boxShadow: "0 10px 30px rgba(31, 41, 55, 0.06)" }}
                >
                  <div className="flex items-start justify-between gap-4">
                    <span className="rounded-full border px-3 py-1 text-xs" style={{ borderColor: "var(--color-border)", color: "var(--color-text-muted)" }}>
                      {continueTerm.meta}
                    </span>
                  ...
                  <p className="mt-5 text-sm font-medium" style={{ color: "var(--color-accent)" }}>
                    查看术语详情 →
                  </p>
                </Link>
              </div>
