      ...
      <main className="mx-auto w-full max-w-2xl flex-1 px-5 py-8">
        ...
        <article className="mt-8 space-y-8">
          ...
          <PolicySection title="一、我们收集什么">
            <p>学习进度（例如章节状态、闪卡评级、复习时间和练习结果）默认保存在你当前设备的浏览器本地存储中。网站不会要求你提供姓名、身份证件、支付信息或通讯录。</p>
          ...
          <PolicySection title="三、第三方服务">
            <p>网站可能使用 Google Analytics 进行匿名或汇总统计。Google 可能按照其自身隐私政策处理相关技术数据。你可以通过浏览器设置限制 Cookie 或网站存储，也可以使用浏览器扩展阻止统计脚本。</p>
          </PolicySection>

          <PolicySection title="四、数据保存与删除">
            <p>本地进度会一直保存在你的浏览器中，直到你清除网站数据或主动删除。清除浏览器数据可能导致本设备上的学习进度无法恢复；这不会影响其他设备上的数据。</p>
            <p>网站升级时会以追加方式保存新的章节进度，不会主动删除或重置既有知识点记录。</p>
          </PolicySection>
          ...
          <section className="rounded-lg border p-4 text-xs leading-6" style={{ borderColor: "var(--color-border)", background: "var(--color-bg-soft)", color: "var(--color-text-muted)" }}>
            当前版本以本地学习进度为主，不提供账号数据导出或跨设备同步功能。相关功能上线前会另行更新说明。
          </section>
        </article>
      </main>
      <Footer />
