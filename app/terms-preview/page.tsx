        ...
        <div className="mx-auto max-w-6xl px-5 py-8 sm:px-8 lg:py-12">
          ...
          <section className="mt-8 overflow-hidden rounded-[28px] border" style={{ borderColor: "#dedede", background: "rgba(250,250,250,.8)", boxShadow: "0 18px 50px rgba(0,0,0,.06)" }}>
            ...
            <div className="border-b px-6 py-5 sm:px-10" style={{ borderColor: "#dedede", background: "rgba(255,255,255,.72)", backdropFilter: "blur(12px)" }}>
              <label className="relative block"><span className="sr-only">搜索术语</span><span aria-hidden="true" className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-lg" style={{ color: "#888" }}>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索所有篇章的术语" className="h-12 w-full rounded-2xl border bg-white/80 pl-11 pr-4 text-sm outline-none transition focus:border-[#777]" style={{ borderColor: "#d8d8d8" }} /></label>
            </div>
            ...
            <div className="space-y-4 px-6 py-7 sm:px-10 sm:py-9">
              {visibleChapters.map((chapter) => {
                ...
                return <section key={chapter.id} className="overflow-hidden rounded-[22px] border bg-white/65" style={{ borderColor: "#dedede", boxShadow: isExpanded ? "0 12px 28px rgba(0,0,0,.05)" : "none" }}>
                  <button type="button" onClick={() => setExpandedChapter(isExpanded && !normalizedQuery ? "" : chapter.id)} className="flex w-full items-start justify-between gap-5 px-5 py-5 text-left sm:px-6" aria-expanded={isExpanded}>
                    ...
                    <span className="flex shrink-0 items-center gap-3 pt-1 text-xs" style={{ color: "#777" }}><span>{chapter.terms.length} 个术语</span><span className={`text-lg transition-transform ${isExpanded ? "rotate-45" : ""}`} aria-hidden="true">＋</span></span>
                  </button>
                  {isExpanded && <div className="border-t px-5 py-5 sm:px-6" style={{ borderColor: "#e8e8e8", background: "rgba(248,248,248,.55)" }}><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{chapter.terms.map((term) => <Link key={term.id} href={term.href} className="group rounded-2xl border bg-white/80 p-4 transition hover:-translate-y-0.5 hover:shadow-[0_10px_22px_rgba(0,0,0,.08)]" style={{ borderColor: "#dedede" }}><div className="flex items-start justify-between gap-3"><p className="text-xl font-semibold tracking-[-0.03em]">{term.code}</p><span className="text-base" style={{ color: "#999" }} aria-hidden="true">↗</span></div><p className="mt-2 text-sm font-medium">{term.name}</p><p className="mt-1 truncate text-xs" style={{ color: "#888" }}>{term.english}</p><p className="mt-3 line-clamp-2 text-xs leading-5" style={{ color: "#666" }}>{term.summary}</p><p className="mt-3 text-[10px]" style={{ color: "#999" }}>{term.meta}</p></Link>)}</div></div>}
                </section>;
              })}
