"use client";

import ConceptFlashcardDeck from "@/components/flashcards/ConceptFlashcardDeck";
import { settlementConcepts } from "@/data/settlement-concepts";
import type { SettlementConcept } from "@/lib/types";

const STORAGE_KEY = "tradelearn-settlement-progress";

/**
 * 国际结算闪卡 — ConceptFlashcardDeck 的薄封装。
 * 定义结算概念的卡片正面/背面内容。
 */
export default function SettlementFlashcardDeck() {
  return (
    <ConceptFlashcardDeck<SettlementConcept>
      concepts={settlementConcepts}
      getId={(c) => c.id}
      storageKey={STORAGE_KEY}
      moduleId="settlement"
      title="结算闪卡"
      emptyTitle="暂无待复习卡片"
      emptyMessage={(s) =>
        `当前没有需要复习的结算知识卡片。已掌握 ${s.mastered} / ${settlementConcepts.length} 个知识点。`
      }
      homeRoute="/settlement"
      renderFront={(concept) => (
        <div className="text-center space-y-3">
          <span className="text-5xl">{concept.icon}</span>
          <h2 className="text-2xl font-bold">{concept.title}</h2>
          <p style={{ color: "var(--color-text-secondary)" }}>
            {concept.englishTitle}
          </p>
          <span
            className="inline-block text-xs px-2 py-0.5 rounded"
            style={{
              background: "var(--color-border)",
              color: "var(--color-text-muted)",
            }}
          >
            {concept.category === "payment-method"
              ? "支付方式"
              : concept.category === "document"
                ? "结算单据"
                : "结算基础"}
          </span>
          <p className="text-xs mt-4" style={{ color: "var(--color-text-muted)" }}>
            点击查看答案
          </p>
        </div>
      )}
      renderBack={(concept) => (
        <div className="text-center space-y-3 max-w-sm">
          <p className="text-sm leading-relaxed">{concept.summary}</p>
          <div
            className="pt-3 mt-3 border-t text-xs space-y-1 text-left"
            style={{ borderColor: "var(--color-border)" }}
          >
            {concept.keyFeatures.slice(0, 3).map((f, i) => (
              <p key={i} style={{ color: "var(--color-text-secondary)" }}>
                · {f}
              </p>
            ))}
          </div>
        </div>
      )}
      resultsProps={{
        concepts: settlementConcepts,
        getId: (c) => c.id,
        getTitle: (c) => c.title,
        getSubtitle: (c) => c.englishTitle,
        getIcon: (c) => c.icon,
        getRoute: (id) => `/settlement/${id}`,
      }}
    />
  );
}
